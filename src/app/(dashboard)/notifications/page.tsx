...
import {
  Bell, BellOff, Clock, AlertTriangle, Award,
  BarChart2, ArrowUpRight, BookOpen, CheckCheck, UserCheck,
} from "lucide-react"

type NotificationStatus = "pending" | "sent" | "failed" | "read"
type NotificationType =
  | "report_reminder" | "report_overdue" | "convert_inactive"
  | "convert_assigned" | "sml_certified" | "kpi_published"
  | "escalation" | "digest"
...
const TYPE_COLOR: Record<NotificationType, string> = {
  ...
  convert_assigned: "bg-green-100 text-green-600",
...
export default function NotificationsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return json.data as { notifications: NotificationDoc[]; unreadCount: number }
    },
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {unreadCount > 0 && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50"
          >
        ...
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            {t.count !== undefined && t.count > 0 && (
              <span className={cn("px-1.5 py-0.5 rounded-full text-xs font-semibold",
        ...
        {isLoading ? (
          <div className="p-12 text-center">
            <div className="inline-flex gap-1.5">
              {[0, 1, 2].map((i) => (
                <div key={i} className="w-2 h-2 rounded-full bg-[#1B4F72]/40 animate-bounce"
          ...
          <div className="p-12 text-center text-red-500 text-sm">Failed to load. Please refresh.</div>
        ) : filtered.length === 0 ? (
          <div className="p-12 text-center">
          ...
          <ul className="divide-y divide-gray-100">
            {filtered.map((n) => {
              ...
              return (
                <li key={n._id}
                  onClick={() => { if (isUnread) markMut.mutate({ ids: [n._id] }) }}
                  className={cn(
                    "flex items-start gap-4 px-5 py-4 transition-colors",
                    isUnread ? "bg-blue-50/40 hover:bg-blue-50/80 cursor-pointer" : "hover:bg-gray-50/60"
                  )}>
                  <div className={cn("mt-0.5 shrink-0 w-9 h-9 rounded-full flex items-center justify-center", colorClass)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {isUnread && <span className="w-2 h-2 rounded-full bg-[#E67E22]" />}
                        <span className="text-xs text-gray-400 whitespace-nowrap">{relativeTime(n.createdAt)}</span>
                      </div>
                    </div>
                    <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{n.body}</p>
                    <p className="text-[10px] text-gray-400 mt-1 capitalize">
                      via {n.channel} · {n.type.replace(/_/g, " ")}
                    </p>
                  </div>
                </li>
              )
